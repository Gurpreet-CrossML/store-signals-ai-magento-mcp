...
server.tool(
  ...
  `,
    ...
    async ({ query, session_id, store_code, page_size = 4, current_page = 1, full_details = false }) => {
        try {
            ...
            const searchResponse = await callMagentoApi("POST", "", graphqlQuery, store_code, true);
            ...
            if (result?.products?.length === 0) {
                ...
                for (let q of keywords) {
                    ...
                    const searchResponse = await await callMagentoApi("POST", "", gQuery, store_code, true);
  ...
  `,
    ...
    async ({ product_id, store_code }) => {
        try {
            ...
            if (!productResponse?.data?.products?.items || productResponse?.data?.products?.items?.length === 0) {
                return {
                    content: [
                        {
                            type: "text",
            ...
            const formattedResults = await formatProducts(productResponse?.data?.products?.items, true);
  ...
  `,
    ...
    async ({ session_id, store_code, sort_key }) => {
        try {
            ...
            const response = await callMagentoApi("POST", "", graphqlQuery, store_code, true);
  ...
  `,
    {
        ...
        order_id: z.number().describe("The unique ID of the order to track. Order ID must be exactly 10 digits long"),
        session_id: z.string().describe("Session identifier"),
    ...
    async ({ email, order_id, session_id, customer_id = "" }) => {
        ...
        try {
            ...
            if (!order) {
                return {
                    content: [
                        { type: "text", text: `We couldn't find an order with the number #${order_id}. Please check the order number and try again.` }
                    ],
                    isError: true,
                };
            }
            ...
            if (order?.customer_email !== email) {
                return {
                    content: [
                        { type: "text", text: "We couldn't find an order matching the provided order number and email address. Please verify both details and try again." }
                    ],
                    isError: true,
                };
            }
